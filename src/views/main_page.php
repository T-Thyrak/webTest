                ...
                <div class="c-navbar-inner">
                    ...
                    <div class="c-ralign">
                        ...
                        <div class="c-nav-item">
                            <button onclick="toggleAddProduct(<?php echo $GLOBALS['user_id'] ?>)" class="btn btn-secondary">
                                <i class="bi bi-node-plus"></i>
                                <?php echo $GLOBALS['tr']->get('add_product') ?>
                            </button>
                        </div>
                        <div class="c-nav-item">
                            <div class="settings">
...
<script>
    ...
    async function toggleAddProduct(user_id) {
        const { value: formValues } = await Swal.fire({
            ...
            <table class="w-100">
            <tr>
            <td class="text-left"><label for="user_id">User ID :</label></td>
            <td><input id="add_user_id" class="swal2-input" name="user_id" value=${user_id} readonly></td>
            </tr>
            <tr>
            <td class="text-left"><label for="product_name">Product name :</label></td>
            <td><input id="add_product_name" class="swal2-input" name="product_name" value=""></td>
            </tr>
            <tr>
            <td class="text-left"><label for="product_qty">Product qty :</label></td>
            <td><input id="add_product_qty" class="swal2-input" name="product_qty" value=""></td>
            </tr>
            <tr>
            <td class="text-left"><label for="product_price">Product price :</label></td>
        ...
        if (formValues) {
            ...
            if (!nameValidity || !qtyValidity || !priceValidity || !qtyContainsOnlyNumber || !priceContainsOnlyNumber) {
                Swal.fire('Denying to add product!', "Please check your input", 'error');
                return;
            } else {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        if (resp == 200) {
                            ...
                            Swal.fire('Forbidden!', "You don't have permission to add this data", 'error');
                        } else {
                            Swal.fire('Error!', "Something went wrong", 'error');
                        }
                    } else {
                        Swal.fire('Cancelled', 'Your changes wasn\'t saved', 'info');
                    }
                });
            }
        } else {
